...
// Estrutura para decodificar a saída JSON do subfinder
type Dados struct {
...
func init() {
	...
	// Monta o nome do arquivo de saída com extensão .json
	saida = "subfinder-" + x + ".json"
}
...
func executa() {
	...
	// Ajuste das flags do subfinder
	command := fmt.Sprintf(
		"docker run --rm --name %s -v %s:/data kali-recon subfinder -d %s -o /data/%s -json",
