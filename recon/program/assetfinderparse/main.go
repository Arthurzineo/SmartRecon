...
func init() {
    ...
    saida = fmt.Sprintf("assetfinder-%s.txt", x)
}
...
func executa() {
    ...
    command := fmt.Sprintf(
        "docker run --rm --name %s -v %s:/data kali-recon assetfinder -subs-only %s >> %s",
        containerName, hostPath, domain, filepath.Join(hostPath, saida),
    )
