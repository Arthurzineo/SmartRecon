...
func init() {
	...
	// Define os headers e a URL
	headers = map[string]string{
		"Accept":       "application/json",
		"Content-Type": "application/json",
	}
	elasticURL = fmt.Sprintf("https://localhost:9200/%s-subdomain/_search", target)
	authUser = "admin"
	authPass = "StrongAdmin123!"

	// Inicializa o mapa
	dicIP = make(map[string]string)
}

func consultaSubdomain() error {
	...
	req, err := http.NewRequest("GET", elasticURL, bytes.NewBuffer(jsonData))
	if err != nil {
	...
	for _, hit := range hits {
		...
		if serverIP != "0.0.0.0" && serverDomain != "" {
			if _, exists := dicIP[serverDomain]; !exists {
				dicIP[serverDomain] = serverIP
			}
		}
	}
...
func parallel() error {
	...
	logFilePath := filepath.Join("/recon/data", target, "temp", "httpx_parallel.log")
	...
	for sub, ip := range dicIP {
		line := fmt.Sprintf("/recon/program/scripts/httpx_parse %s %s %s\n", target, sub, ip)
...
func main() {
	err := consultaSubdomain()
	if err != nil {
		fmt.Printf("Erro na consultaSubdomain: %v\n", err)
		os.Exit(1)
	}
