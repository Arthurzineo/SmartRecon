...
func init() {
	...
	dicSistemas = make(map[string][]string)
}
...
func consulta() error {
	...
	for _, hit := range hits {
		...
		if _, exists := dicSistemas[urlOriginal]; !exists {
			...
			serverPort := fmt.Sprintf("%v", source["server.port"])
			urlPath, _ := source["url.path"].(string)

			dicSistemas[urlOriginal] = []string{serverDomain, serverPort, urlPath}
		}
	}
...
func parallel() error {
	...
	logFilePath := filepath.Join("/recon/data", target, "temp", "nuclei_parallel.log")
	...
	for sis, data := range dicSistemas {
		serverDomain := data[0]
		serverPort := data[1]
		urlPath := data[2]

		line := fmt.Sprintf("/recon/program/scripts/nuclei_parse %s %s %s %s %s\n",
			target, sis, serverDomain, serverPort, urlPath)
	...
	fmt.Println("[+] PROCESSANDO NUCLEI \n")
